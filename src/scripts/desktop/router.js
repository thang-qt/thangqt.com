...
const windowStartPosition = { left: 36, top: 64 };
const windowWorkAreaPad = 12;
...
function defaultWindowRect(count, windowSpec) {
  const stage = getStage();
  ...
  const maxLeft = Math.max(windowWorkAreaPad, stageRect.width - size.width - windowWorkAreaPad);
  const maxTop = Math.max(windowStartPosition.top, stageRect.height - size.height - windowWorkAreaPad);
  ...
  left = Math.max(windowWorkAreaPad, Math.min(left, maxLeft));
...
function shouldBleedWindow(href) {
